...
import { EnhancedTaskDetailModal } from '../task/EnhancedTaskDetailModal'
import { EnhancedTaskCreateModal } from '../task/EnhancedTaskCreateModal'
import { formatHours, formatHoursWithSuffix, formatHoursVariance } from '../../utils/hourFormatter'
import toast from 'react-hot-toast'
import { 
  Plus, Search, Filter, Grid, List, BarChart3, Calendar, Users, Target, 
  Clock, CheckCircle, AlertCircle, ArrowRight, Edit2, Trash2,
  MoreVertical, Play, Pause, Timer, Flag, User, Eye, TrendingUp
} from 'lucide-react'
...
const ProjectTasksTab: React.FC<ProjectTasksTabProps> = ({
  project,
  users,
  onTaskUpdate,
  onTaskCreate,
  ...
  // Create user lookup from users prop
  useEffect(() => {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/tasks/?project_id=${project.id}`, {
        headers: {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/tasks/analytics/summary?project_id=${project.id}`, {
        headers: {
          'Authorization': `Bearer ${tokens?.access_token}`,
          'Content-Type': 'application/json'
        }
    ...
    if (assigneeFilter !== 'all') {
      filtered = filtered.filter(task => 
        assigneeFilter === 'unassigned' 
          ...
          : task.assignee_id === assigneeFilter || (task.assignee_ids && task.assignee_ids.includes(assigneeFilter))
      )
    }
  ...
  useEffect(() => {
    fetchTasks()
    fetchAnalytics()
  }, [tokens?.access_token, project.id])

  useEffect(() => {
    if (viewMode === 'kanban') {
      fetchKanbanData()
    }
  }, [viewMode, kanbanView, tokens?.access_token, project.id])
  ...
  return (
    <div className="space-y-6" data-testid="project-tasks-tab">
      {/* Header */}
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">
            Tasks for {project.name} ({filteredTasks.length})
          </h2>
...
// Kanban Board Component - Reused from TasksPage
...
// Tasks List Component - Reused from TasksPage
...
// Task Analytics Component - Reused from TasksPage
...
// Time Log Modal Component - Reused from TasksPage
